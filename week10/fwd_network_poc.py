import os
from week10.config import HOME_DIR
from week10.utilities.logic import make_forward_network
import networkx as nx


def run():
    OUTPUT_DIR = os.path.join(HOME_DIR, "PycharmProjects", "smo", "week10", "data")
    SEED_LIST_NAME = "indian_military_nationalism"
    START_DATE = "2022-01-01"
    END_DATE = "2026-01-01"

    G = make_forward_network([SEED_LIST_NAME], START_DATE, END_DATE)

    nx.write_graphml(G, os.path.join(OUTPUT_DIR, f"{SEED_LIST_NAME}_fwd_network.graphml"))

if __name__ == '__main__':
    run()
