...
def post_time_series_chart_api(
    start_date: str, end_date: str, unit: str, seed_list_names: list[str]
) -> list[dict]:
    resp = requests.post(
...
def post_make_forward_network_api(
    ...
    def post_make_domain_network_api(
        ...
        B.add_nodes_from([(node["id"], node) for node in nodes])
        print(f"domain graph has {len(B.nodes())} nodes and {len(B.edges())} edges")
